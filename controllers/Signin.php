...
class Signin extends CI_Controller {
	...
	function autologin(){
		if ($this->Users_model->login_user_id() > 1) {
			$this->load->model("Attendance_model");
			$this->load->model("Users_model");
			date_default_timezone_set('Asia/Kolkata');
			$now = new DateTime();
			$date = $now->format('Y-m-d');
			$in_time = convert_time_to_24hours_format($now->format('H:i'));
			...
			if($save_id){
				$_SESSION['save_id']=$save_id;
				$_SESSION['in_date_time']= $in_date_time;
				$_SESSION['time'] = time();
			}				
		}
	}
	...
	function autologout(){
		...
		$date = $now->format('Y-m-d');
		$in_time = convert_time_to_24hours_format($now->format('H:i'));
		$out_date_time = $date . " " . $in_time;
		if (isset($_SESSION['save_id'])){
			$data = array(
				"user_id" => $_SESSION['user_id'],
				"save_id" =>  $_SESSION['save_id'],
				"in_date_time" =>  $_SESSION['in_date_time'],
				"out_date_time" =>  $out_date_time,
			);
			$this->Attendance_model->autologin($data);
		}
	}
